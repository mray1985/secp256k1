#!/usr/bin/env python3
"""
F-20260709-04 — Band-floor + doubled x-coordinate X2(Q).

Preregistered before eval. Sibling of F-03:
  T = 3 Qx^2 * (2 Qy)^{-1} mod p
  X2 = T^2 - 2 Qx mod p
...
CANDIDATE_ID = "F-20260709-04"
OUT = OUT_DIR / "F-20260709-04_band_floor_doubled_x_result.txt"
PREREG_MD = OUT_DIR / "prereg" / "F-20260709-04_band_floor_doubled_x.md"
...
def doubled_x(qx: int, qy: int) -> int | None:
    """X2(Q) = T^2 - 2 Qx mod p, T = 3 Qx^2 (2 Qy)^{-1}."""
    inv = modinv((2 * qy) % P_FIELD)
    if inv is None:
        return None
    T = (3 * qx * qx * inv) % P_FIELD
    return (T * T - 2 * qx) % P_FIELD
...
def score_x2(rows) -> float:
    xs, ys = [], []
    for r in rows:
        x2 = doubled_x(r.px, r.py)
        if x2 is None:
            continue
        xs.append(lead_frac(r.d))
        ys.append(lead_frac(x2))
    if len(xs) < 8:
        return 0.0
...
def main() -> None:
    ...
    rows = [r for r in rows if doubled_x(r.px, r.py) is not None]
    print(f"Band-floor rows with X2 defined: {len(rows)}")
    ...
    res = evaluate(
        "F-20260709-04 band_floor_doubled_x",
        score_x2,
        rows,
        prereg=prereg,
    ...
    payload = {
        ...
        "closed_branch": "Qx+Qy feature + Mersenne offsets FALSIFIED (F-01/F-02)",
        "prior_sibling": "F-03 T(Q) FAIL",
        "this_fork": "X2(Q)=T^2-2Qx with band floor",
        "result": asdict(res),
