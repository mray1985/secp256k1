...
def power_of_two_decomposition(N):
    """
    Decompose integer N into a sum of c * 2^k terms
    using a greedy minimal-term strategy.
    Coefficients c may be > 1.
    """
    if N < 0:
...
def pretty_print_powers(terms, remainder):
    """
    Human-readable string for power-of-two decomposition
    """
    expr = " + ".join(
        f"{c}·2^{k}" if c > 1 else f"2^{k}"
        for c, k in terms
    )
    if remainder:
        expr += f" + {remainder}"
...
for idx in sorted(puzzles):
    value = puzzles[idx]
    terms, remainder = power_of_two_decomposition(value)

    print(f"\nPuzzle {idx}:")
    print(f"Value: {value}")
    print(pretty_print_powers(terms, remainder))
