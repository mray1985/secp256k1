# triple_double_add_double.py
# Simulates the repeating 3-2-1-2 pattern over your known puzzle values
...
# --- pattern setup  ---
pattern = [3, 2, 1, 2]   # triple, double, add, double
pattern_index = 0

def next_value(prev_vals):
    """Apply the next step of the 3-2-1-2 pattern to recent numbers."""
    global pattern_index
    coeff = pattern[pattern_index % len(pattern)]
    pattern_index += 1
    # simple recurrence: new = coeff * last + previous
    # you can refine this weighting to match your empirical observations
    return coeff * prev_vals[-1] + prev_vals[-2]

# --- generator  ---
def generate_puzzles(start_index=70, end_index=80):
    vals = [puzzles[i] for i in sorted(puzzles.keys())]
    for n in range(start_index+1, end_index+1):
        new_val = next_value(vals[-2:])
        puzzles[n] = new_val
        vals.append(new_val)
    return puzzles

# --- run example  ---
predicted = generate_puzzles(70, 75)
for k in range(71, 76):
    print(f"Puzzle {k}: {predicted[k]}")
