# ============================================================
# Oscillatory Generator Descent Sequence
# Generator rule: puzzle n uses puzzle (n - 3)
# ============================================================

# ---- GIVEN SEQUENCE (ground truth) ----
SEQ = {
...
# ------------------------------------------------------------
# Oscillatory construction using a fixed generator g
# ------------------------------------------------------------
def oscillatory_sum(target, basis):
    remaining = target
    terms = []

    # descend generators largest → smallest
    for g in sorted(basis, reverse=True):
        while True:
            if remaining >= g:
                terms.append(g)
                remaining -= g
...
# ------------------------------------------------------------
# Build next value using generator = seq[n-3]
# ------------------------------------------------------------
def build_next(seq, n):
    generator = seq[n - 3]
    basis = [seq[i] for i in range(1, n)]

    # empirical: ~3 oscillations of g
    target = 0
    while target < generator * 3:
        target += generator + 2 * generator

    value, terms = oscillatory_sum(target, basis)
    return value, terms


# ------------------------------------------------------------
# Continue sequence forward
# ------------------------------------------------------------
def continue_sequence(start_n, end_n):
    seq = dict(SEQ)

    for n in range(start_n, end_n + 1):
        if n - 3 not in seq:
            raise ValueError(f"Missing generator for n={n}")

        value, terms = build_next(seq, n)
        seq[n] = value
...
# ------------------------------------------------------------
# EXAMPLE: extend beyond 130
# ------------------------------------------------------------
if __name__ == "__main__":
    continue_sequence(66, 71)
